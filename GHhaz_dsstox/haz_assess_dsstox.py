#!/usr/bin/env python3
"""
QHA DSSTox - PubChem + EPA CompTox (DSSTox) + OPERA hazard assessment.

Adds DSSTox/ToxValDB to QHA Pro. Requires EPA CompTox API key (see README).

API key: Create comptox_api_key.txt from comptox_api_key.txt.example, or set
COMPTOX_API_KEY environment variable.

Usage:
  python haz_assess_dsstox.py 67-64-1
  python haz_assess_dsstox.py 67-64-1 -o report.json
  python haz_assess_dsstox.py --list compounds.csv -o reports/
"""
...
from dsstox_client import load_api_key, process_chemical as dsstox_process
from hazard_merge import merge_comptox_into_hazard_data
...
def _summarize_comptox(c: dict) -> dict:
    """Extract key DSSTox fields for display."""
    out = {"dsstox_dtxsid": c.get("DTXSID"), "dsstox_error": c.get("Error"), "dsstox_preferred_name": c.get("preferredName")}
    td = c.get("ToxicityData") or {}
    ld50_list = td.get("ld50") or []
    if ld50_list:
        it = ld50_list[0]
        n = it.get("toxvalNumeric")
        u = it.get("toxvalUnits") or ""
        sp = it.get("speciesCommon") or ""
        rt = it.get("exposureRoute") or ""
        out["dsstox_ld50"] = f"{n} {u} {sp} {rt}".strip()
    else:
        out["dsstox_ld50"] = ""
    out["dsstox_carcinogenicity"] = f"{len(td.get('carcinogenicity') or [])} records"
    out["dsstox_genotoxicity"] = f"{len(td.get('genotoxicity') or [])} records"
    return out
...
def process_one_cas(
    ...
    """Full assessment: PubChem + DSSTox + OPERA with provenance."""
    cas = cas.strip()
    result = {
        "cas": cas,
        "name": cas,
        "status": "PubChem + DSSTox",
        "error": None,
        "pubchem": {},
        "dsstox": {},
        "opera": {},
    ...
    # DSSTox (CompTox)
    comptox = dsstox_process(cas, "CAS", api_key)
    result["dsstox"] = _summarize_comptox(comptox)
    if comptox.get("preferredName"):
        result["name"] = comptox["preferredName"]
    if not comptox.get("Error"):
        merge_comptox_into_hazard_data(haz, comptox, prefer_experimental=True)
        result["status"] = "PubChem + DSSTox"
    time.sleep(0.3)
    ...
    if run_opera:
        try:
            ...
            if op.get("opera_available") and op.get("error") is None:
                result["status"] = "PubChem + DSSTox + OPERA"
...
def flatten_for_csv(assessment: dict) -> dict:
    row = {
        ...
        "DSSTox_LD50": assessment.get("dsstox", {}).get("dsstox_ld50"),
...
def main() -> int:
    import argparse
    p = argparse.ArgumentParser(description="QHA DSSTox - PubChem + EPA CompTox + OPERA")
    ...
    p.add_argument("--api-key-file", default=None, help="Path to comptox_api_key.txt (default: script dir)")
    ...
    if not api_key:
        print("ERROR: EPA CompTox API key required.", file=sys.stderr)
        print("  1. Copy comptox_api_key.txt.example to comptox_api_key.txt", file=sys.stderr)
        print("  2. Replace YOUR_API_KEY_HERE with your key (get free key: ccte_api@epa.gov)", file=sys.stderr)
        print("  Or set COMPTOX_API_KEY environment variable.", file=sys.stderr)
        return 1
    ...
    for i, cas in enumerate(cas_list):
        ...
        r = process_one_cas(cas, api_key=api_key, opera_exe=str(exe_path) if exe_path else args.opera_exe, java_home=args.java_home, run_opera=not args.no_opera)
        results.append(r)
        if not args.quiet and len(cas_list) == 1:
